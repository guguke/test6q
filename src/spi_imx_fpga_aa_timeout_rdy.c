...
#define DRIVER_NAME_RX "spi_imx_rx"
...
static unsigned int gCountISRrcv=0;
static unsigned int gCountSetupxRcv=0;
...
#define SPI_BUF_LEN  0x100000
#define SPI_BUF_LEN32 (SPI_BUF_LEN>>2)
#define SPI_BUF_MASK  (SPI_BUF_LEN-1)
#define SPI_BUF_MASK32 (SPI_BUF_MASK>>2)
struct spi_imx_data {
...
#define MXC_SPI_BUF_RX1(type)						\
static void spi_imx_buf_rx1_##type(struct spi_imx_data *spi_imx)		\
{									\
	unsigned int val = readl(spi_imx->base + MXC_CSPIRXDATA);	\
        u32 *p32;                                                       \
        unsigned int offset;                                            \
...
#define MXC_SPI_BUF_TX1(type)						\
static void spi_imx_buf_tx1_##type(struct spi_imx_data *spi_imx)		\
{									\
	type val = 0;							\
        u32 *p32;                                                       \
        unsigned int offset;                                            \
...
#define MXC_SPI_BUF_RX0(type)						\
static void spi_imx_buf_rx0_##type(struct spi_imx_data *spi_imx)		\
{									\
	unsigned int val = readl(spi_imx->base + MXC_CSPIRXDATA);	\
									\
}

#define MXC_SPI_BUF_TX0(type)						\
static void spi_imx_buf_tx0_##type(struct spi_imx_data *spi_imx)		\
{									\
	type val = 0;							\
									\
	spi_imx->count -= sizeof(type);					\
		...
		struct spi_imx_config *config)
{
	u32 ctrl = SPI_IMX2_3_CTRL_ENABLE, cfg = 0;
        ...
        //trace_printk(" func._config     spi .. rdy : %d   bpw:%d \n",spi_imx->rdy,config->bpw);
	/*
	...
	if (config->mode & SPI_CPOL)
		cfg |= SPI_IMX2_3_CONFIG_SCLKPOL(config->cs)
		 	| SPI_IMX2_3_CONFIG_SCLKCTL(config->cs);
...
static void spi_imx_push_rx(struct spi_imx_data *spi_imx)
{
	while (spi_imx->txfifo < spi_imx->devtype_data.fifosize) {
		if (!spi_imx->count) break;
                //trace_printk("    func _push  tx    txfifo:%d\n",spi_imx->txfifo);
				 ...
				 struct spi_transfer *t)
{
	...
	//trace_printk(" %s bpw:%d cfg_speed:%d cfg_mode:0x%x cfg_len:%d\n",__func__,bpw,config.speed_hz,config.mode,len);
        ...
        else if(config.speed_hz != spi_imx->cfg_old.speed_hz) flagDoConfig=1;
        else if(config.mode != spi_imx->cfg_old.mode ) flagDoConfig=1;
     
	//trace_printk(" in:%d out:%d \n",spi_imx->rxin,spi_imx->rxout);
        //flagDoConfig = 1;
        if(flagDoConfig){
          if(0x0ff == spi_imx->cfg_old.bpw ){
	      //trace_printk("   first rx.config \n");
              spi_imx->cfg_old.bpw = config.bpw;
              spi_imx->cfg_old.speed_hz = config.speed_hz;
              spi_imx->cfg_old.mode = config.mode;
              spi_imx->cfg_old.cs = config.cs;
          ...
          spi_imx->cfg_old.mode = config.mode;
          spi_imx->cfg_old.cs = config.cs;
          spi_imx->cfg_new.bpw = config.bpw;
          spi_imx->cfg_new.speed_hz = config.speed_hz;
          spi_imx->cfg_new.mode = config.mode;
				 ...
				 struct spi_transfer *t)
{
	...
	//spi_imx->tx = spi_imx_buf_tx_u32;
	  ...
	  //clk_enable(spi_imx->clk);
	    ...
	    wait_for_completion_timeout(&spi_imx->xfer_done,1);
          }
#endif
	  spi_imx->devtype_data.config(spi_imx, &config);
...
int tx2buf(void *p,int len, struct spi_imx_data *spi_imx)
{
        int i;
        int len32=len>>2;
...
static int spi_imx_transfer_rx(struct spi_device *spi,
				struct spi_transfer *transfer)
{
	...
	if(spi_imx->newConfig){
	  ...
	  spi_imx->devtype_data.intctrl(spi_imx, MXC_INT_RR | MXC_INT_TE);
	   ...
	   wait_for_completion_killable_timeout(&spi_imx->xfer_done,HZ);
...
static int __devinit spi_imx_probe(struct platform_device *pdev)
{
	...
	master->num_chipselect = mxc_platform_info->num_chipselect;
        ...
        p = kmalloc(SPI_BUF_LEN,GFP_KERNEL);
        if(!p){
          printk(" tx . spi:%d kmalloc memory error \n",master->bus_num);
        }
        gptx[master->bus_num] = p;
        p = kmalloc(SPI_BUF_LEN,GFP_KERNEL);
        if(!p){
          printk(" rx . spi:%d kmalloc memory error \n",master->bus_num);
        }
        gprx[master->bus_num] = p;

	spi_imx = spi_master_get_devdata(master);
        printk(" probe done . spi.bus_num:%d   ****   addr_spi_imx:0x%08x   sizeof_spi_imx_data:%d\n",master->bus_num,spi_imx,sizeof(struct spi_imx_data));
        if(master->bus_num != 2 ) spi_imx->rdy=1;// init  ,  fixme
        else spi_imx->rdy=0;
	...
	spi_imx->flag_55aa = 0x55aaaa55;
        ...
        spi_imx->rxonly = 0;
        if(master->bus_num==1) spi_imx->rxonly = 1;
        spi_imx->rxstop = 0;
	...
	init_completion(&spi_imx->xfer_done);
        ...
        if(spi_imx->rxonly){
	  spi_imx->rx = spi_imx_buf_rx1_u32;// read only
	  spi_imx->tx = spi_imx_buf_tx0_u32;
        }
        else{
	  spi_imx->rx = spi_imx_buf_rx0_u32;// read nothing
	  spi_imx->tx = spi_imx_buf_tx1_u32;
        }

        spi_imx->cfg_old.bpw=255;
